...
class HomeVC: UIViewController,UICollectionViewDataSource, UICollectionViewDelegate, MFMailComposeViewControllerDelegate {
    ...
    let likenotification = Notification.Name("LikeIdentifier")
    var userType = false
    var userId:String!
    
    var postsResults = [JSON]() {
        didSet {
    ...
    override func viewDidLoad() {
        ...
        refreshControl.addTarget(self, action: #selector(HomeVC.PullRefresh), for: UIControlEvents.valueChanged)
        self.collectionView.addSubview(refreshControl)
        
        if let name = prefs.string(forKey: userNameKeyConstant) {
            ...
            case "FAVORITE POSTS":
                ...
                break
            case "FEATURED POSTS":
                self.navigationItem.title = "FEATURED POSTS"
                break
            case "MY POST":
...
//        self.validator.showGSMessage(message: "Connecting....", fromController: self.view, typeOfStyle: .success)
    ...
    func collectionView(_ collectionView: UICollectionView, cellForItemAt indexPath: IndexPath) -> UICollectionViewCell {
        ...
        if let images = postsResults[indexPath.row]["post_tags"].string  {
        ...
        let underlineAttributedString = NSAttributedString(string: "By #\(postsResults[indexPath.row]["first_user"].stringValue)", attributes: underlineAttribute)

         cell.firstName.attributedText = underlineAttributedString
         cell.firstName.handleHashtagTap { hashtag in
         self.userType = false
         self.performSegue(withIdentifier: "profile", sender: indexPath)
         }
 
        let underlineAttributedString2 = NSAttributedString(string: "#\( postsResults[indexPath.row]["second_user"].stringValue)", attributes: underlineAttribute)
        ...
        cell.postTitle.text = "\( postsResults[indexPath.row]["post_name"].stringValue)"
        cell.descriptionTxt.text = "\( postsResults[indexPath.row]["description"].stringValue)"
        cell.firstImageView.sd_setImage(with: NSURL(string:   postsResults[indexPath.row]["first_prof_image"].stringValue) as URL!, placeholderImage: nil)
        cell.secondImageView.sd_setImage(with: NSURL(string:   postsResults[indexPath.row]["second_prof_image"].stringValue) as URL!, placeholderImage: nil)
        cell.sddDateLabel.text = "\( postsResults[indexPath.row]["added_post"].stringValue) by \(postsResults[indexPath.row]["first_user"].stringValue)"
        switch postsResults[indexPath.row]["isLike"].stringValue {
        case "Y":
            cell.likeBtn.select()
        default:
        ...
        if indexPath.row == postsResults.count - 1 { // last cell
    ...
    override func prepare(for segue: UIStoryboardSegue, sender: Any?) {
     //var d1 = ["post_details": "post_details"]
        if segue.identifier == "details" {
        let destinationVC : DetailVC = segue.destination as! DetailVC
        let indexPath = sender as! NSIndexPath
        destinationVC.details = JSON(["post_details":self.postsResults[indexPath.row]])
//            merge[self.postsResults[indexPath.row]]
        }
        ...
        if segue.identifier == "profile" {
            ...
            switch userType {
            case true:
                userId = self.postsResults[indexPath.row]["second_user_id"].stringValue
            default:
                userId = self.postsResults[indexPath.row]["user_id"].stringValue
            }
    ...
    func getPosts(pG:String) {
        ...
        if let name = prefs.string(forKey: userNameKeyConstant) {
            switch name {
            case "TIMELINE":
                api = "\(ConstantsAPI.HoosRightServer)featured_post.php?user_id=\(userId!)&limit=\(pG)"
                break
            case "FAVORITE POSTS":
                api = "\(ConstantsAPI.HoosRightServer)favorite_all_user_post.php?user_id=\(userId!)&limit=\(pG)"
                break
            case "FEATURED POSTS":
                api = "\(ConstantsAPI.HoosRightServer)featured_post.php?user_id=\(userId!)&limit=\(pG)"
                
            case "MY POST":
                api = "\(ConstantsAPI.HoosRightServer)user_post.php?user_id=\(userId!)&limit=\(pG)"
                
            default:
                
                //api = "\(ConstantsAPI.HoosRightServer)favorite_all_user_post.php?user_id=1&limit=0"
                api = "\(ConstantsAPI.HoosRightServer)featured_post.php?user_id=\(userId!)&limit=\(pG)"
            }
        }
        else {
    ...
    func getNextPage() {
        ...
        if let name = prefs.string(forKey: userNameKeyConstant) {
            switch name {
            case "TIMELINE":
                api = "\(ConstantsAPI.HoosRightServer)featured_post.php?user_id=\(userId!)&limit=\(pageNumber)"
                break
            case "FAVORITE POSTS":
                api = "\(ConstantsAPI.HoosRightServer)favorite_all_user_post.php?user_id=\(userId!)&limit=\(pageNumber)"
                break
            case "FEATURED POSTS":
                api = "\(ConstantsAPI.HoosRightServer)featured_post.php?user_id=\(userId!)&limit=\(pageNumber)"
                
            case "MY POST":
                api = "\(ConstantsAPI.HoosRightServer)user_post.php?user_id=\(userId!)&limit=\(pageNumber)"
        ...
        requestManager.getRandomPosts(path:api, onCompletion: {
            ...
            if json == [JSON.null] {
                ...
                self.postsResults = self.requestManager.postResults
            }
    ...
    @IBAction func likeAction(_ sender: DOFavoriteButton) {
        ...
        if let row = (sender as AnyObject).tag {
            if self.postsResults[row]["isLike"].string == "N" {
            ...
            let parameters = [
                "user_id"    : userId!,
                "post_id" : self.postsResults[row]["id"].string!,
                "friend_id" : self.postsResults[row]["second_user_id"].string!
            ]
            print("api:\(api!)")
            print("parameters:\(parameters)")
            RestApiManager.sharedInstance.postRandomPosts(path: api!, Parameters: parameters as [String : AnyObject], onCompletion: {
                json ->Void in
                let index = json[0]["post_details"]["isLike"]
                switch index {
                case "Y"  :
                    self.postsResults[row].dictionaryObject = json[0]["post_details"].dictionaryObject
    ...
    @IBAction func moreAction(_ sender: Any) {
        ...
        let report = UIAlertAction(title: "Report", style: UIAlertActionStyle.default) { (action) -> Void in
            if !MFMailComposeViewController.canSendMail() {
                ...
                self.composeMail(ReportId:self.postsResults[row!]["id"].string!)
            }
        }
        alertController.addAction(report)
        

        if self.postsResults[row!]["user_id"].string! == userId! || self.postsResults[row!]["second_user_id"].string! == userId! {
            print("User Id:\(userId) & user_id:\(self.postsResults[row!]["user_id"]) & second user id : \(self.postsResults[row!]["second_user_id"])")
                ...
                self.requestManager.postRandomPosts(path: self.api!, Parameters: parameters as [String : AnyObject]!, onCompletion: {
                    ...
                    case 1,0  :
                        ...
                        if index == 1 {
                            ...
                            self.postsResults[row!].arrayObject?.remove(at: row!)
                            self.postsResults = self.postsResults.filter { $0 != JSON.null }
                        }
                    case JSON.null  :
...
class CustomCell: UICollectionViewCell, TagListViewDelegate {
    ...
    @IBOutlet weak var sddDateLabel: UILabel!
