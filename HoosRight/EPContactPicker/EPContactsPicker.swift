...
open class EPContactsPicker: UITableViewController, UISearchResultsUpdating, UISearchBarDelegate,MFMessageComposeViewControllerDelegate {
    ...
    override open func tableView(_ tableView: UITableView, didSelectRowAt indexPath: IndexPath) {
        ...
        if multiSelectEnabled {
            ...
            for phoneNumber in selectedContact.phoneNumbers {
                print("The \(phoneNumber.phoneLabel) number of \(phoneNumber.phoneNumber)")
                self.name += [phoneNumber.phoneNumber]
            }
            print("ContactNumber::\(self.name[0])")
            if !MFMessageComposeViewController.canSendText() {
                print("SMS services are not available")

            }
            else {
                self.sendMessage(number: self.name[0])
            }
    ...
    func sendMessage(number:String) {
        ...
        messageVC.body = "Checkout this new cool App. HoosRight"
        messageVC.recipients = [number] // Optionally add some tel numbers
